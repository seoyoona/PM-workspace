...
2. **Notion**: PM Action Hub DB (data_source_id: `a183aae9-a894-8379-8708-87cf507ec8e8`) — 신규 PM 할 일 자동 등록 (Step 8)
3. **Teams**: 개발팀 dev 채팅 (`TEAMS_CHAT_{CLIENT}_DEV`) — 영어 status update 메시지
4. **터미널**: 저장/전송 확인
...
- 1 → Step 7 (Notion 저장) + Step 8 (PM Hub 자동 등록) + Step 9 (Teams 전송)
- 2 → Step 7 + Step 8 만 실행. dev-chat은 복사 안내만 출력 + activity-log에 skill="daily-scrum" 1회 기록
- 3 → 수정 요청 받은 후 Notion/dev-chat **+ PM Hub 추출 룰** 모두 재적용해 재생성 → 다시 이 프롬프트
- 4 → 중단 (저장/전송/PM Hub 등록/로그 모두 skip)
...
### Step 8. PM Action Hub 자동 등록 (필수, 옵션 1·2 모두 적용)

Daily Scrum의 **"오늘 할 일" 섹션 항목 중 PM 본인이 직접 처리할 일**을 PM Action Hub DB에 개별 액션으로 자동 등록한다. 이 단계는 사용자 추가 확인 없이 항상 실행된다 (Step 6 옵션 1·2 선택 시).

**대상 항목 (PM 할 일만):**
- "오늘 할 일" 섹션의 bullet 중, **PM이 직접 실행할 액션**만 포함
- "Team today" 섹션 (개발팀 작업)은 제외
- 메모/Blocker 섹션은 제외 (블로커는 PM 액션이 아님)

**중복 방지:**
- Step 2에서 PM Action Hub로부터 가져온 항목 (`상태 = 오늘 / 진행 중`)은 **이미 등록된 상태이므로 재등록 금지**
- 등록 직전, 같은 프로젝트 + 같은 제목(`[ProjectName] X` exact match) + 작성일(today)으로 PM Action Hub에 존재하는지 1회 확인
  ```json
  {"and": [
...
**Properties 매핑:**
- `제목` (title): `[ProjectName] 태스크명` (Daily Scrum의 bullet 텍스트를 그대로, prefix 없으면 자동 추가)
- `프로젝트` (select): {project-name}
- `상태` (select): "오늘"
- `액션 유형` (select): bullet 내용 기반 자동 분류
  - "전달", "공유", "회신", "deliver", "client", "고객" → `고객 커뮤니케이션`
  - "확인", "체크", "정리", "업데이트", "검토" → `운영 체크`
  - 그 외 (팀과의 sync, follow-up, 미팅 등) → `내부 follow-up`
- `출처` (select): "manual"

**호출:**
- `mcp__notion-cigro__notion-create-pages` 단일 호출로 모든 항목 batch 생성
- parent: `{"type": "data_source_id", "data_source_id": "a183aae9-a894-8379-8708-87cf507ec8e8"}`

**예외:**
- "오늘 할 일"에 PM 신규 항목이 0건이면 (모두 PM Action Hub에서 가져온 것) → 등록 skip, 그대로 다음 단계 진행
- 등록 실패 시 → 경고 출력 + Step 9 (Teams 전송) 진행 (Teams 전송과 독립)
- batch 호출이 partial success (n건 중 k건 실패)이면 성공 항목은 그대로 유지하고 실패 항목 수를 §11 터미널 출력에 명시 ("PM Hub 등록 실패: {k}건 — 수동 보정 필요"). PM이 수동 retry/보정. 자동 retry 금지.

### Step 9. Teams 전송 (옵션 1 선택 시)
...
🎯 PM Action Hub 신규 등록: {N건} (skip: {M건 — 이미 존재})
⚠️ PM Hub 등록 실패: {k}건 — 수동 보정 필요   ← partial success 시에만 출력
...
- **Daily Scrum 저장 후 PM 할 일은 무조건 PM Action Hub에 자동 등록 (Step 8)** — 추가 확인 없이 실행. PM Action Hub에서 가져온 항목은 중복 등록 금지. Team today / Blocker / 메모는 PM Hub로 보내지 않음.
