...
**중복 체크 (C1 멱등성 가드)** — 저장 직전에 반드시 실행:
- **Unique key**: 회의명 + 날짜 + 클라이언트. 회의명은 `--attendee`/참석자 컨텍스트 기반 추론 또는 제목 그대로 ("미팅 노트 — [날짜]"가 기본이므로 같은 날 2건 이상이면 중복 판단).
...
- **결과 분기**:
  ...
  - **1건 이상** → 사용자 확인 (Confirmation Format 준수):
    ...
    2. 새로 생성 (다른 미팅이면)
    3. 취소
    추천: 1 (같은 미팅 재실행일 가능성 높음)
    ```
    - 1 → 가장 최근 1건 archive (`mcp__notion-cigro__notion-update-page` with `archived:true`) → 새로 생성
    - 2 → 중복 허용하고 새로 생성 (오전/오후 별도 미팅 등)
    - 3 → 중단
- **archive 실패 시 생성 중단** — 새 페이지 만들지 않음.

Notion 커뮤니케이션 DB 저장 (`mcp__notion-cigro__notion-create-pages` — data_source_id: `47d3aae9-a894-83bf-8db8-071dd9a16fcd`):
- 제목: "미팅 노트 — [날짜]"
- 클라이언트 / 프로젝트 / 유형: "미팅 노트" / 방향: "Client→Dev" / 작성일: 오늘
- Notion 소스 블록 읽기: `mcp__notion-cigro__notion-fetch` 사용 (page ID로 블록 읽기)
