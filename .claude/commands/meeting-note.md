...
[후보]
  ├─ source_quote 없음 → 폐기
  ├─ 실행 동사 없음, 사실/상태 서술 → (a) 배경 / 신뢰 관리 컨텍스트
  ├─ "어떨까 / 고민 / 의논 중" 논의 상태 → (b) 논의 메모 / 진행 상태
  ├─ 합의·방침 명시, 실행 주체 없어도 됨 → (c) 결정 / 합의 사항
  ├─ 실행 동사 + owner 명시 or 문맥상 분명 → (d) Action Items
  │    ├─ 블로커/"오늘/당장/최우선" 명시 → priority=🔴
  │    ├─ "검토·판단·확인·승인" + 명시적 요청 주체 → priority=🔵
  │    └─ 나머지 → priority=🟡
  ├─ "이번엔 안 함 / 나중에" 명시 → (e) 후순위 / 유지보수
  ├─ 신규 기능 + 스코프 미확정 → (f) 스코프 경계 (이번 라운드 아님)
  └─ owner/action/요청 주체 중 2개 이상 불명 → (g) 논의 메모 / 진행 상태 (Action 승격 금지)
```
...
**priority 보수 규칙:**
- 🔴: 블로커·고장·"오늘/당장/최우선" 발화 **명시**. 근거 없으면 쓰지 않음.
- 🟡: 실행 동사 + owner 확정 + 블로커 아님.
- 🔵: **"누군가가 확인·정리·응답·승인해야 하는 명시 요청"일 때만**. 단순 "owner 불명" 이유로 🔵 사용 금지.
- **Action Item 미등록**: owner/action/요청 주체 중 2개 이상 불명확하면 `논의 메모`로. "일단 🔵에 넣자" 금지.
...
## 핵심 요약
- 3-4줄. Action Items 🔴 + 결정/합의 항목에서만 도출.

## 결정 / 합의 사항
- 명시 합의된 방향·사양만. 액션 없어도 기록.
- "…진행 중" 같은 진행 상태 bullet 금지 (→ 논의 메모로).
...
(표 컬럼은 위 5개만. confidence/status/source_quote는 내부 보관, 표 미노출.
 `[추론]`은 Action 텍스트 인라인 태그로만 표시.
 근거 없는 하위 그룹은 통째로 생략.)

## 스코프 경계 — 이번 라운드 아님
- 고객사 티켓 정리 예정 항목. "지금 착수 아님" 선언.

## 후순위 / 유지보수
- 명시적으로 미룬 실행 항목만.

## 논의 메모 / 진행 상태
- "BE와 의논 중" / "방식 검토 중" 같은 in-flight.
- [추론] 태그 항목도 여기.
- owner/action 불명확하여 Action Item 승격 안 된 항목도 여기.

## 배경 / 신뢰 관리 컨텍스트 [PM 참고용]
- 전략 맥락. Part2/3에 노출하지 않음.
```

**각 섹션은 내용 없으면 생략.**

**source_quote는 Notion에 저장하지 않음** — Phase B 내부 검증용으로만 사용. 검증 의심 시 터미널에 별도 덤프 가능.
...
**Part 1의 Action Items 테이블 + 결정/합의 + 논의 메모에서 쿼리해 생성.** 표를 그대로 복제하지 않고 **Teams 톤 서술 bullet**으로 풀어쓴다.

| 쿼리 | → Part 2 섹션 |
|---|---|
| Action Items 🔴 WHERE owner IN (Dev, Dev+*) | Critical context (2~3줄 bullet) |
| Action Items 🟡 WHERE owner IN (Dev, Dev+*) + 결정/합의 dev 대상 | This round (3~5 bullet) |
| 논의 메모 WHERE dev 대상 | Discussion/open (1~2줄, 기본 생략) |
| 후순위/유지보수 WHERE owner=Dev | Maintenance later (해당 시만) |

**제외:** owner=Client만, owner=PM만, 배경/신뢰 관리 컨텍스트.
...
| 쿼리 | → Part 3 섹션 |
|---|---|
| 핵심 요약 (고객 가시) | 오늘 논의된 핵심 내용 |
| 결정/합의 사항 (고객 가시) | 확정된 사항 |
| Action Items WHERE owner IN (Dev, PM) + status (고객 가시) | 진행 예정 사항 |
| Action Items 🔵 WHERE owner=Client | 추가 확인 부탁드리는 사항 |
| 스코프 경계 | 위 "진행 예정" 끝에 "4차 신규 건은 별도 티켓으로 정리 부탁드립니다" 한 줄 |

**제외:** 내부 버그 수정 디테일, PM 내부 액션, 개발팀 내부 기술 변경, 일정 지연 사유, 팀 리소스, 신뢰 관리 컨텍스트, [추론] 태그.
...
### 핵심 원칙
- **Action Items 테이블이 single source of truth** — 모든 액션은 테이블에 있고, prose에서 재서술 금지.
- **근거 없는 필드는 null** — `source_quote` 없는 액션은 생성 금지. owner/due는 녹취 명시 시에만.
- **Action Item 승격 문턱 높게** — 모호하면 `논의 메모`로. 개수가 줄어드는 건 회귀 아님.
...
- `confidence=low` + `[추론]` 액션이 4차 신규·스펙 미확정 성격이면 Action Items 대신 `스코프 경계`로 보내는 것을 우선 검토.
- Dev 액션 중 2일 이상 / 여러 컴포넌트 + owner·스펙 명확 → /to-spec 추천 태그.
- 해당 없는 섹션/하위 그룹은 생략.
- 용어는 glossary 기준.

### Part별 언어/톤
- Part 1: 한국어, 간결체. Action Items 표는 가볍게.
- Part 2: 영어, Direct/practical. 표 복제 금지, 서술 bullet.
