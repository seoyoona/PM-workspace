...
- **시나리오 ID 메타 보존 (v1.5)** — 입력 본문 상단에 `시나리오 ID: SCN-{project}-{NNN}` 패턴이 있으면 출력 ticket 본문 첫 줄에 같은 메타 라인을 그대로 보존한다. 제거·수정 금지. `/qa-scenario` 자동 호출 ❌, `/issue-ticket` 자동 호출 ❌, Notion DB property 추가 ❌ — 메타는 본문 텍스트로만 유지 (v1.6에서 property화 검토).
