...
3. 각 항목에서 추출: 페이지(위치), 수정내용, 스크린샷 유무, 요청일자, 검수회차, **원본 페이지 URL/ID**

### Step 2.5: 중복 체크 (C1 멱등성 가드)

**Unique key**: 원본 QA 페이지 URL (고객 QA DB 각 row의 page URL).

각 항목 처리 전에 내부 Tasks DB에서 기존 티켓 조회:
1. Step 1에서 확보한 Tasks DB 스키마에서 "출처 URL" 저장 필드 확인:
   - **URL 타입 필드 있음** (예: `출처`, `source URL`, `ref`): 해당 필드 기준 exact match
   - **없음**: Title 접미사 또는 description에 `[source: <URL>]` 태그 포함 여부로 판단
2. 각 고객 QA 항목의 원본 URL로 `mcp__notion-cigro__notion-fetch` 조회
3. **결과 분기** (항목별):
   - **매치 0건** → 신규 티켓으로 미리보기에 포함
   - **매치 1건 이상** → **skip + 알림** (티켓 생성 X). 미리보기 테이블에 별도 행:
     ```
     | # | 원문 요약 | 상태 | 기존 티켓 |
     |---|----------|------|----------|
     | 3 | 홈 배너 | ⏭️ 중복 skip | [기존 링크] |
     ```

**핵심 원칙**:
- 같은 고객 QA row에서 파생된 티켓이 이미 있으면 **무조건 skip** (덮어쓰기/업데이트 금지 — 개발자가 이미 작업 중일 수 있음)
- 고객이 같은 내용을 다른 row에 다시 올린 경우 (다른 URL) → 별도 티켓 생성 (skip 안 됨)
- skip된 항목은 Step 5 "고객 QA DB Status 변경"에서도 제외

### Step 3: 미리보기 (필수)
**티켓을 바로 생성하지 않음.** 먼저 변환 결과를 테이블로 보여줌:

```
## QA 피드백 변환 미리보기 (신규 {n}건 / 중복 skip {m}건)

| # | 원문 요약 | → Title (EN) | Priority | QA Category | 상태 |
|---|----------|-------------|----------|-------------|------|
| 1 | 홈화면 배너 안보임 | Home banner not displaying | High | UI Bug | 신규 |
| 2 | 결제 오류 | Payment error on checkout | 🔥Highest🔥 | Logic Bug | 신규 |
| 3 | 로그인 버튼 | — | — | — | ⏭️ 중복 skip ([기존 링크]) |
```
...
- **중복 방지 (C1 멱등성 가드)** — 원본 QA 페이지 URL 기준 기존 티켓이 있으면 skip + 알림. 덮어쓰기/업데이트 금지 (개발자가 작업 중일 수 있음). Step 2.5 참조.
...
- **QA Plan / Scenario ID 메타 보존** — 입력 본문 상단에 다음 패턴 중 하나라도 있으면 출력 ticket 본문 첫 줄에 같은 메타 라인을 그대로 보존한다. 제거·수정 금지.
  - `QA Plan: QA-<CLIENT>-YYYYMMDD` (`/qa-plan` 산출 plan ID)
  - `Scenario: P0-NN` / `P1-NN` / `EDGE-NN` / `REG-NN` (`/qa-plan` 내부 시나리오 ID)
  - `시나리오 ID: SCN-<project>-NNN` (구 `/qa-scenario` 메타, 호환 유지)
- `/qa-plan` 자동 호출 ❌, `/issue-ticket` 자동 호출 ❌, Notion DB property 추가 ❌ — 메타는 본문 텍스트로만 유지 (v1.6에서 property화 검토).
