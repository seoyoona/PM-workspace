#!/usr/bin/env bash
# /change-brief v1 자동 검증 하네스
# 외부 write 없음 (tests/change-brief/outputs/ 로컬 파일만 검사)
# Usage: bash scripts/test-change-brief.sh
...
# Section extraction: from "### <name>" until next "###" or "##"
extract_section() {
  local file="$1"
...
echo "=========================================="
echo "/change-brief v1 자동 검증 하네스"
...
for case_id in "${CASES[@]}"; do
  ...
  # case07: expects no output file
  if [ "$case_id" = "case07" ]; then
