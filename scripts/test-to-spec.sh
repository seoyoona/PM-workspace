#!/usr/bin/env bash
# /to-spec v2 자동 검증 하네스 (4-bucket triage gate + PM confirm gate 흡수 후)
# 외부 write 없음 (tests/to-spec/outputs/ 로컬 파일만 검사)
# Usage: bash scripts/test-to-spec.sh
...
TESTS_DIR="tests/to-spec"
OUTPUTS_DIR="$TESTS_DIR/outputs"
CHECKS_DIR="$TESTS_DIR/checks"

PASS=0
FAIL=0
FAIL_LOG="$(mktemp -t test-to-spec-fail.XXXXXX)"
trap 'rm -f "$FAIL_LOG"' EXIT

# Find output file matching case ID prefix (case08 expected to return empty)
...
run_global_check() {
  ...
  case "$check_type" in
    must_match)
      ...
      echo "$case_id [global]: must_match FAIL — $description (pattern: $pattern)" >> "$FAIL_LOG"
      return 1
      ;;
    must_not_match)
      if grep_pattern "$pattern" "$content"; then
        echo "$case_id [global]: must_not_match FAIL — $description (matched: $pattern)" >> "$FAIL_LOG"
        return 1
      fi
      return 0
      ;;
    *)
      echo "$case_id [global]: unknown check type '$check_type'" >> "$FAIL_LOG"
      return 1
      ;;
  esac
}
...
run_case_check() {
  ...
  case "$f1" in
    must_match)
      ...
      echo "$case_id: must_match FAIL — $f3 (pattern: $f2)" >> "$FAIL_LOG"
      return 1
      ;;
    must_not_match)
      if grep_pattern "$f2" "$content"; then
        echo "$case_id: must_not_match FAIL — $f3 (matched: $f2)" >> "$FAIL_LOG"
        return 1
      fi
      return 0
      ;;
    must_match_in_section)
      ...
      echo "$case_id: must_match_in_section FAIL — $desc (section: $section_name, pattern: $section_pattern)" >> "$FAIL_LOG"
      return 1
      ;;
    file_must_not_exist)
      local matches
      ...
      echo "$case_id: file_must_not_exist FAIL — $f3 (found: $matches files matching $f2)" >> "$FAIL_LOG"
      return 1
      ;;
    must_match_filepath)
      if [ -f "$f2" ]; then
      ...
      echo "$case_id: must_match_filepath FAIL — $f3 (missing: $f2)" >> "$FAIL_LOG"
      return 1
      ;;
    note)
      return 0
      ;;
    *)
      echo "$case_id: unknown check type '$f1'" >> "$FAIL_LOG"
      return 1
      ;;
  esac
}
...
echo "=========================================="
echo "/to-spec v2 자동 검증 하네스 (4-bucket triage + PM confirm gate)"
...
for case_id in "${CASES[@]}"; do
  ...
  if [ "$case_id" = "case08" ]; then
    if [ -n "$output_file" ]; then
      ...
      FAIL=$((FAIL+1))
      echo "$case_id: client 누락 시 outputs/에 파일 생성되어 있음 — 3순위 fallback 위반" >> "$FAIL_LOG"
      continue
    fi
  ...
  if [ -z "$output_file" ]; then
    ...
    FAIL=$((FAIL+1))
    echo "$case_id: outputs/ 디렉토리에 ${case_id}-*.md 없음" >> "$FAIL_LOG"
    continue
  fi
