...
- **meeting-note** = source of truth → dev-chat / client-chat은 downstream. **Action Items 테이블 기반** (🔴 미팅 직후 긴급 / 🟡 실행 / 🔵 확인 필요), 결정/합의·논의 메모·스코프 경계 섹션 분리. 2-phase 파이프라인(Phase A Raw 추출 → Phase B 분류 매핑). `source_quote` 없는 액션 생성 금지, owner/action/요청 주체 중 2개 이상 불명이면 `논의 메모`로 보수 승격. 표 컬럼 최소화(`#/Owner/Action/Due/Context`) — confidence/status/source_quote는 내부 보관. Part2/3은 쿼리 결과를 표 복제 X, Teams/카톡 톤으로 풀어씀.
